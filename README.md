...
```bash
VERSION=2026.06.28.4 PYTHON=python3 ./scripts/build-release.sh
```

## Local Unraid test

After the `2026.06.28.4` GitHub release exists, copy the repository to `/mnt/user/temp/plugins/liquidctl`, then run:
...
After publishing the `2026.06.28.4` release, install this URL from **Plugins > Install Plugin**:

```text
https://github.com/SkippyTheLost/unraid-liquidctl/releases/latest/download/liquidctl.plg
```

Creating the matching `2026.06.28.4` Git tag runs the build and publishes both required release assets automatically.

The bundled settings icon is a PNG version of the official `liquidctl_logo_v1_circle_256` asset from the [liquidctl repository](https://github.com/liquidctl/liquidctl).
